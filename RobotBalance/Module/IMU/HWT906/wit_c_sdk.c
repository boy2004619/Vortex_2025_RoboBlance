...
WIT_JY901S_IMU WIT_JY901S_IMUData;
...
{
	HAL_UART_Transmit(&huart7,p_data,uiSize,0xFFFF);
}
...
void WIT_JY901S_Dispose(void)
{
	if(s_cDataUpdate)
	{
	 for(int i = 0; i < 3; i++)
		{
			WIT_JY901S_IMUData.fAcc[i]   = sReg[AX+i] / 32768.0f * 16.0f;
			WIT_JY901S_IMUData.fGyro[i]  = sReg[GX+i] / 32768.0f * 2000.0f;
			WIT_JY901S_IMUData.fAngle[i] = sReg[Roll+i] / 32768.0f * 180.0f;
		}
	}
}
