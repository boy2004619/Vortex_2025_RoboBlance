...
#define YS_RecvData_Size     18u

#define YS_Reset_T	            2.0f
#define YS_Allow_Error_T	    0.15f //电机复位时允许的力矩误差
#define YSMotor_RR	            6.33f
...
typedef struct
{
    ...
    float Pos_output;                   //输出端角度 = Pos/YSMotor_RR
...
typedef struct
{
    ...
    float Reset_Pos;                    //复位状态时编码器的值
    float Pos_output;                   //输出端角度 = Pos/YSMotor_RR
    float W_output;                     //输出端角速度 = W/YSMotor_RR
    float Amend_Pos;                    //修正后的电机角度 （rad）
} MOTOR_recv;


typedef struct
{
    MOTOR_send YSMotor_Send[4];
    MOTOR_recv YSMotor_Receive[4];

    Motor_Working_Type_e stop_flag; // 四个关节电机的启停标志
    bool YSMotor_Reset;				//宇树电机开机复位标志位

} YSMotor_Control;

extern MOTOR_send cmd;   //以全局变量声明电机控制结构体和电机数据结构体，方便在故障时通过debug查看变量值
extern MOTOR_recv data;
extern YSMotor_Control Joint_Motor; //关节电机的总结构体
...
void YS_Motor_Control_test(void);
#endif
